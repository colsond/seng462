...
//  pthread_t threads;
  ...
  if (argc == 1) {
    ...
    for(conn_num=0;conn_num<conn_max;conn_num++) {
      pthread_create(&threads[conn_num],NULL,(void*)&connector,&count[conn_num]);
//      pthread_join(threads[conn_num], NULL);
    } //conn for loop
    for(conn_num=0;conn_num<conn_max;conn_num++) {
//      pthread_create(&threads[conn_num],NULL,(void*)&connector,&count[conn_num]);
      pthread_join(threads[conn_num], NULL);
...
//    sprintf(outBuffer,"zYx,abcdefghij\n\0",msg_num);
    sprintf(outBuffer,"zYx,%5.5d-%4.4d\n\0",conn_num+1,msg_num+1);
